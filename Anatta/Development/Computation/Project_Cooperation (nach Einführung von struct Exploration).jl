...
#-----------------------------------------------------------------------------------------
"""
	setSP!(players::Players)

Helper function for changing the sucker's payout S in the players payoff matrix.
"""
function setSP!(S, players::Players)
	players.payoff[1, 2] = S
end
...
#-----------------------------------------------------------------------------------------
"""
	investigate(exp::Exploration)

Explore which strategies survive.
"""
function investigate(exp::Exploration)

	avghistory = [[0.0,0.0] for _ in 1:exp.nMutations+1]	# History averaged over ntrials.
	...
	avghistory = avghistory ./ exp.nTrials					# Normalize results
end

#-----------------------------------------------------------------------------------------
"""
	investigateSP(exp::Exploration, sRange::StepRange)

Repeated investigation of developmental trajectories with different sucker's payouts
for the payoff matrix.
"""
function investigateSP(exp::Exploration, sRange::StepRange)
	
	sVect = collect(sRange)
	nTrajectories = length(sVect)
	trajectories = Vector{Vector}(undef, nTrajectories)

	for i in 1:nTrajectories
		setSP(sVect[i], exp.players)
		trajectories[i] = investigate(exp)
	end
...
function present_multiple_graphs!(fig, trajectories, nCols = 2)
	...
	for i in 1:nTrajectories
		...
		scatterlines!( axis, getindex.(trajectories[i],1), getindex.(trajectories[i],2),
			color=:blue,
			markercolor=:red, markersize=10
		)
		xlims!(axis, 0, 1)
...
function demo()
	...
	trajectories = investigateSP(exp, sRange)

	fig = Figure()

	present_multiple_graphs!(fig, trajectories, nCols)

	fig

	# TODO spáter einkommentieren
	# present(history)
end
