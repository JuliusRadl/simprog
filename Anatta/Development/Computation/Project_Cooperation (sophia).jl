...
"""
function mutate!( players::Players, mu::Float64=0.2, epsilon::Float64=0.05)		#epsilon löschen
	...
	nBestStrats = nStrategies - nWorstStrats
    ...
    # Adjust frequencies
    #players.x[worstStratsIndex] .= sum(players.x[worstStratsIndex]) / nWorstStrats
	players.x[worstStratsIndex] .= 0.01 		#(1/nStrategies)
	remaining_freq = 1 - sum(players.x[worstStratsIndex])
	players.x[bestStratsIndex] .= remaining_freq * (players.x[bestStratsIndex] / sum(players.x[bestStratsIndex]))
...
function novak!( players::Players)
	...
	# Assigning the long term payoff matrix
	players.novak = A
end
...
function behaviour( strategy::Vector)
    # TODO: Auf Strategiematrix anpassen
	p,q = strategy
...
function present( history::Vector)
	...
	save("CooperationInfoSheet.png",fig)		#.pdf
	nothing
end
...
function unittest()
	...
	ntrials = 50
	nStrategies = 100									# How many strategy types?
	nMutations = 1000									# How many mutation events occur?
	nGenerations = 20									# How many generations between mutations?
	mu = 0.1			#0.01						# Cutoff for low-fitness strategies
	
	varEpsilon = 0.1									# Random strategy variation
	initStrat = [0.075, 0.075]							# Initial strategy for set!
	initEpsilon = 0.025									# Variation of initial strategies
	...
	set!(players, initStrat, initEpsilon)					# Set up initial exploitative population
	# Main.Infiltrator.@infiltrate
	println("Initial population:")
	show(players, 0.1)									# Show initial population
	for _ in 1:ntrials
		# Perform a single trial simulation:
		set!(players, initStrat, initEpsilon)				# Reset initial exploitative population
		avghistory[1] += avgstrategy(players)

		for mut in 1:nMutations
			# Mutate then simulate nGenerations:
			mutate!(players, mu)						# Replace current weakest fraction of strategies
			# vary!(players, varEpsilon)				# Slightly change strategies
			# redistribute!(players, varEpsilon)		# Redistribute frequencies slightly
	...
	scatterlines!( axis, getindex.(avghistory,1), getindex.(avghistory,2),
		color=:blue,
		markercolor=:red, markersize=10
	)
	xlims!(axis, 0, 1)
...
function demo(ntrials=50)
	...
	for _ in 1:ntrials
		...
		for mut in 1:nMutations
			# Mutate then simulate nGenerations:
			mutate!(players, mu)		#0.001				# Replace current weakest fraction of strategies
			simulate!( players, nGenerations)			# Iterate PD interactions
			avghistory[mut+1] += avgstrategy(players)	# Accumulate new simulation results
		end
	end
