...
#-----------------------------------------------------------------------------------------
"""
	check_limit_point(pop_start, growth_rate, max_steps)

Author: @Julius Radl
Iterates the logistic function for a start value and a growth rate.
You can easily check for convergence by the output.

Peinlich: Ich hab eigentlich nur die Funktion nestlist
weiter oben umgeschrieben, aber es ist mir nicht aufgefallen...
"""
function check_limit_point(pop_start, growth_rate, max_steps)
    step = 0
    x = pop_start
	L(r) = (y -> r * y * (1-y))
    while !(L(growth_rate)(x) == x) & (step <= max_steps)
        x = L(growth_rate)(x)
        println(x)
        step = step + 1
    end
