...
# Done: Add graphics backend package
...
function schelling( preference=1.0; extent=(60,60))
    ...
    # Done: Set n_agents to 80% of the total number of grid points
    n_agents = round(Int, prod(extent) * 0.8, RoundDown)
    for n in 1:n_agents
        # Done: Place Persons of random tribe
        add_agent_single!( schelling_model; tribe=mod(n-1, 2))
    end
...
function agent_step!( me::Person, model)
    ...
    # Done: Decide how to react
    me.comfort = proportion_mytribe ≥ model.preference

    # Done: If uncomfortable or on occasion, jump to a random empty grid location
    ...
    for (i,preference) in enumerate(preferences)
        for _ in 1:ntrials
            ...
            for agent in allagents(abm)
                ...
                n_tribal_nbrs = 0
                # Done: Count tribally similar neighbours

                for nbr in nbrs
                    if agent.tribe == nbr.tribe
                        n_tribal_nbrs += 1
                    end
                end
...
"""
function demo(preference=0.8)
    # Cap preference values above and below:
    preference = max(0.0,min(1.0,preference))
    abm = schelling(preference)

    # Calculate mean x position of tribes
    tribe_0_x(agent) = (agent.tribe == 0 ? agent.pos[1] : 0)
    tribe_1_x(agent) = (agent.tribe == 1 ? agent.pos[1] : 0)
    function partialmean(data)
        nonzero_data = [x for x in data if x != 0]
        sum(nonzero_data) / length(nonzero_data)
    end

    # Done: Define tribe position data
    adata = [(:comfort, sum), (tribe_0_x, partialmean), (tribe_1_x, partialmean)]
    agent_df, model_df = run!( abm, 99; adata)
    agent_df

    # Done: Generate video output
    ...
    # Done: Create an exploratory playground
    ...
    # Present insights as line graph
    present_insight()
