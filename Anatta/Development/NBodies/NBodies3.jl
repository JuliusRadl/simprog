...
function simulate( nb::NBody)
	...
	# Initialisation:
	x[1] = nb.x0 # x[1]: Position aller Körper zum Zeitpunkt 1
	p[1] = nb.p0
...
function animate( nb::NBody, t, x)
	...
	#=
	Julius' Kommentar:
	x[1] enthält die Positionen aller Körper zum Zeitpunkt 1,
	die jeweils aus einem [x, y, ...]-Koordinatenvektor bestehen
	x[2] ... zum Zeitpunkt 2 ...
	D. h., wenn wir die x-Koordinate aller Körper zum Zeitpunkt 1 haben wollen,
	müssen wir von jedem Element in x[1] das erste Element entnehmen.
	Das geht mit map(bodycoords -> bodycoords[1], x[1])
	(Grundform: map(element -> element[1], elements)),
	würde aber auch mit [bodycoords[1] for bodycoords in x[1]] gehen.

	Da steht also: x_current ist ein Observable, das einen Vektor enthält,
	der die x-Koordinaten aller Körper zum Zeitpunkt 1 enthält. Nicht mehr.
	Das map ist hier nur für diese einmalige Startbelegung von x_current wichtig.
	=#
	...
	#=
	Julius' Kommentar:
	Hier hängen wir einen Listener an unsere Observables x_current, y_current
	und timestamp. Dieser Scatterplot & Text wird auf Änderungen in den
	Observables reagieren.
	=#
