...
function simulate( nb::NBody)
	...
	for n = 1:nb.nsteps
		...
		p[n+1] = p[n] + nb.dt * force_on_masses(x[n],nb.m)	# Accelerate each body with a force.
	end
...
function force_on_masses( locations::Vector, masses::Vector)
	...
	#=
	Julius' Kommentar:
	Wir stellen uns vor, dass wir eine Matrix erzeugen wollen,
	in der die Spalten von i bis j benannt sind und die 
	Zeilen ebenso. Die Zellen stehen für den Abstand der
	beiden Punkte i und j. Indem wir die transposed Matrix
	von der ursprünglichen Matrix abziehen, erhalten wir in
	der Hauptdiagonale überall 0. i soll ja auch keinen
	Abstand zu i haben.
	=#
