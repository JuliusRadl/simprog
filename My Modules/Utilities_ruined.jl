...
#-----------------------------------------------------------------------------------------
"""
	eratosthenes_good( maximum::UInt)

Generate a list of prime numbers up to a user specified maximum maximum. This implementation ports
directly from Java to Julia, and is therefore unreasonably low-level.
...
Changes:
Rename variables
Shorter multiplication notation
...
function eratosthenes(N)
    ...
    if N >= 2
        ...
        # sieve
        crossout_prime_multiples!(is_prime)
        ...
        # return null array if bad imput
        return Int[]
    end
end
#----------------------------------------------------------------------------------------------
"""
	crossout_prime_multiples!(prime_array)
	
Helper function for the sieve of eratosthenes.
Modifies an existing boolean array by crossing out
multiples of all primes numbers in the array.
"""
function crossout_prime_multiples!(prime_array::BitVector)
    j = 0
    N = length(prime_array)
    for n in 2:round(Int, sqrt(N) + 1)
        if prime_array[n] # if i is uncrossed, cross its multiples
            for j in 2n:n:N
                prime_array[j] = false # multiple is not a prime
            end
        end
    end
end
